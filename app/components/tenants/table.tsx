...
import { DELETE_TENANT } from '@/app/utils/queries';
...
function EnhancedTableToolbar(props: EnhancedTableToolbarProps) {
  ...
  const [res2, executeQuery] = useQuery({query: DELETE_TENANT, variables: {ids: selected}, pause: true
  });

  const delete_tenants = () => {
    // Delete User Query
    setIsLoading(true)
    executeQuery()
    setIsLoading(false)
...
export default function EnhancedTable({data}: {data: any[]}) {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = data.map((n: { node: { id: string; }; }) => n.node.id);
